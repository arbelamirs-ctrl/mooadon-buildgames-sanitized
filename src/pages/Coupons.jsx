...
export default function Coupons() {
  ...
  const [manualForm, setManualForm] = useState({ discount_type: 'percentage', discount_value: 10, max_uses: 1, expires_days: 30 });
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (coupon) => (
        <div className="flex items-center gap-1 font-semibold text-teal-400 text-sm">
  ...
  const createManualCoupon = async () => {
    ...
    try {
      ...
      const couponCode = `MAN-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
