...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Mark setup as in-progress + start 7-day trial ───────────────────────
    ...
    await base44.asServiceRole.entities.Company.update(companyId, {
      ...
      // ✅ DEFAULT: every new company starts on Fuji Testnet with onchain enabled
    ...
    if (!refreshedCompany.blockchain_wallet_address) {
      ...
      if (!walletCreated) {
        ...
        return Response.json({ success: false, error: errMsg, companyId, blockchain_warning: errMsg });
      }
    }
    ...
    if (existingTokens.length === 0) {
      try {
        ...
        console.warn('Step 4.2 FAIL:', e.message);
        await base44.asServiceRole.entities.Company.update(companyId, {
          setup_status: 'error',
          setup_last_error: errMsg,
          onboarding_completed: false
    ...
    try {
      ...
      const emailBody = `
        <div style="font-family: Arial, sans-serif; max-width: 620px; margin: 0 auto; padding: 24px; background-color: #f5f5f5;">
          ...
          <div style="background-color: white; padding: 28px; border-radius: 12px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
            ...
            <p style="font-size: 15px; line-height: 1.65; color: #374151; margin: 0 0 16px;">${introParagraph}</p>
            ...
            <div style="text-align: center; margin-top: 24px;">
              <a href="https://mooadon.base44.app" style="display: inline-block; background: linear-gradient(135deg, #14b8a6, #0891b2); color: white; padding: 13px 32px; text-decoration: none; border-radius: 10px; font-weight: 600; font-size: 15px; letter-spacing: 0.2px;">
      ...
      await base44.asServiceRole.integrations.Core.SendEmail({ to: toEmail, subject, body: emailBody });
